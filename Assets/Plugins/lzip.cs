...
public class lzip {
    ...
    public static int extract_entry(string zipArchive, string arcFilename, string outpath, object fileBuffer = null, ulong[] proc = null, string password = null) {
		...
		var pbuf = gcA(proc);
			...
			if(fileBuffer!= null) {
                if (fileBuffer is byte[]) {
				    ...
				    if(proc != null) res = zipEntry(null, arcFilename, @outpath, fbuf.AddrOfPinnedObject(), FileBuffer.Length, pbuf.AddrOfPinnedObject(), password);
				    else res = zipEntry(null, arcFilename, @outpath, fbuf.AddrOfPinnedObject(), FileBuffer.Length, IntPtr.Zero, password);
                ...
                if (fileBuffer is IntPtr || fileBuffer is inMemory) {
				    ...
				    if(proc != null) res = zipEntry(null, arcFilename, @outpath, FileBuffer, fileBufferLength, pbuf.AddrOfPinnedObject(), password);
				    else res = zipEntry(null, arcFilename, @outpath, FileBuffer, fileBufferLength, IntPtr.Zero, password);
                }
		...
		if(proc != null) res = zipEntry(@zipArchive, arcFilename, @outpath, IntPtr.Zero, 0, pbuf.AddrOfPinnedObject(), password);
		else res = zipEntry(@zipArchive, arcFilename, @outpath, IntPtr.Zero, 0, IntPtr.Zero, password);
		pbuf.Free();
		return res;
		...
		var pbuf = gcA(proc);
			...
			if(fileBuffer != null) {
                ...
                if (fileBuffer is IntPtr || fileBuffer is inMemory) {
                    ...
                    if (proc != null) res = zipEX(null, @outPath, ibuf.AddrOfPinnedObject(), FileBuffer, fileBufferLength, pbuf.AddrOfPinnedObject(), password);
                    else res = zipEX(null, @outPath, ibuf.AddrOfPinnedObject(), FileBuffer, fileBufferLength, IntPtr.Zero, password);
